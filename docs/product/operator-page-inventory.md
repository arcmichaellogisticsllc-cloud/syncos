...
|---|---|---|---|
| `/intelligence/account-onboarding` | Contract-backed workbench | Account Onboarding Workbench now shows the approved prime/customer and contractor/vendor onboarding spine from Identified through Mobilized, preferring the `/account-onboarding` backend read model and falling back to existing organization, contact, candidate, capacity provider, contract, rate schedule, and territory data when needed. | Add onboarding profile detail pages and modalized stage-transition actions later. |
