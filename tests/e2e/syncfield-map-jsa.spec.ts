...
test.describe.serial("P8 SyncField map foundation and Daily JSA", () => {
  ...
  test("Partner Admin sees safe JSA history and P8 keeps Production unavailable", async ({ page }) => {
    ...
    await expect(page.getByText("Daily Production After start authorization")).toBeVisible();
    await expect(page.getByText("not available in P8")).toBeVisible();
  });
});
